...
// QueryOwer chains the current query on the "ower" edge.
func (cq *CarQuery) QueryOwer() *UserQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.M2O, true, car.OwerTable, car.OwerColumn),
		)
...
// WithOwer tells the query-builder to eager-load the nodes that are connected to
// the "ower" edge. The optional arguments are used to configure the query builder of the edge.
func (cq *CarQuery) WithOwer(opts ...func(*UserQuery)) *CarQuery {
	query := (&UserClient{config: cq.config}).Query()
...
func (cq *CarQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Car, error) {
	...
	if query := cq.withOwer; query != nil {
		if err := cq.loadOwer(ctx, query, nodes, nil,
			func(n *Car, e *User) { n.Edges.Ower = e }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CarQuery) loadOwer(ctx context.Context, query *UserQuery, nodes []*Car, init func(*Car), assign func(*Car, *User)) error {
