...
// describe returns a string describing the current token,
// for use in errors.
...
func Parse(input string) (_ Expr, err error) {
	...
	lex.scan.Mode = scanner.ScanInts | scanner.ScanInts | scanner.ScanFloats
	lex.next()
	e := parseExpr(lex)
	if lex.token != scanner.EOF {
		return nil, fmt.Errorf("unexpectec %s", lex.describe())
	}
